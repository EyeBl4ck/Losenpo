...
async def comprar_logins_list(c: Client, m: CallbackQuery):
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton("💳 Cartões", callback_data="comprar_vale"),
                InlineKeyboardButton("🎟 Logins", callback_data="comprar_login unit"),
                
                
            ],
            [
                InlineKeyboardButton("⚙ Ferramentas", callback_data="termo"),
            ],
          
			[
                InlineKeyboardButton("✅ REFERENCIAS ✅", url="t.me/SuporteCardersRobot"),
            ],
    ...
    await m.edit_message_text(
        f"""<a href='https://i.ibb.co/r0jsL20/IMG-20230712-135120-982.jpg'</a><b>🎟️ Escolha abaixo o produto que deseja comprar.</b>
...
async def comprar_logins(c: Client, m: CallbackQuery):
    ...
    organ.append([InlineKeyboardButton(
                    "Buscar Logins por Cidade",
                    switch_inline_query_current_chat="buscarlog_cidade A",
                )])
    
    organ.append([InlineKeyboardButton(text="⬅️ Voltar", callback_data="comprar_log")])
    kb = InlineKeyboardMarkup(inline_keyboard=organ)
    await m.edit_message_text(
        f"""<b>🎟 Qual o tipo de Login que você deseja comprar?</b>
...
async def buy_off_logins(c: Client, m: CallbackQuery):
    ...
    kb = InlineKeyboardMarkup(
                    inline_keyboard=[
                        [
                            InlineKeyboardButton(
                                text="Acessar Loja",url=f"https://t.me/FenixLoginsBot"
                            ),
                        ],
                    ]
                )
    ...
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(text="⬅️ Voltar", callback_data="comprar_log"),
            ],
        ]
    )
    try:
